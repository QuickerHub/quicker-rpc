...
public sealed class QkrpcMcpDocsTools
{
        ...
        int? limit = null)
    {
        ...
        return verb switch
        {
            ...
            "get" => DocsGet(topic ?? string.Empty, reference),
    ...
    private Task<string> DocsGet(string topic, string? reference = null)
    {
        var doc = Guides.GetDoc(topic, reference);
        var payload = new
        {
            ok = doc.Success,
            action = "docs-get",
            ...
            availableReferences = doc.AvailableReferences,
        };
        return Task.FromResult(QkrpcMcpJson.FormatObject(payload));
    }
