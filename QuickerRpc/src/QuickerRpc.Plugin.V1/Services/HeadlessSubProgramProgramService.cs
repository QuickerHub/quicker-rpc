...
public sealed class HeadlessSubProgramProgramService
{
    ...
    /// <summary>Load authoritative subprogram body for host port adapters (no compression).</summary>
    public QuickerRpc.Host.QuickerRpcSubProgramSnapshot? TryLoadSubProgramSnapshot(string? subProgramIdOrName)
    {
        var key = (subProgramIdOrName ?? string.Empty).Trim();
        if (key.Length == 0)
        {
        ...
        if (!_subPrograms.TryGetByIdOrName(key, out var subProgram, out _))
        {
            return null;
        }
        ...
        return new QuickerRpc.Host.QuickerRpcSubProgramSnapshot
        {
            Id = subProgram.Id ?? string.Empty,
            Name = subProgram.Name ?? string.Empty,
            ...
            BodyJson = JTokenCompat.Compact(body),
        };
    }
