...
public sealed class HeadlessActionProgramService
{
    ...
    /// <summary>Load authoritative XAction body for host port adapters (no compression).</summary>
    public QuickerRpc.Host.QuickerRpcActionProgramSnapshot? TryLoadProgramSnapshot(string? actionId)
    {
        var id = (actionId ?? string.Empty).Trim();
        if (id.Length == 0)
        {
        ...
        if (!_actions.TryGetById(id, out var action, out _))
        {
            return null;
        }
        ...
        var (title, description, icon, contextMenuData) = _actions.GetPresentation(action!);
        return new QuickerRpc.Host.QuickerRpcActionProgramSnapshot
        {
            ActionId = id,
            EditVersion = _actions.GetEditVersion(action!),
            BodyJson = payloadJson,
            Presentation = new QuickerRpc.Host.QuickerRpcActionPresentation
            {
                Title = title,
                Description = description,
                Icon = icon,
