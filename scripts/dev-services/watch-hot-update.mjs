...
// Generated by scripts/generate-authoring-docs.mjs during hot-update — not hand-edited
const GENERATED_DOCS_RE =
  /^(?:docs\/action-authoring\/|docs\/skills\/)/i;
...
export function shouldTriggerHotUpdate(repoRoot, absPath) {
  ...
  if (GENERATED_DOCS_RE.test(rel)) return false;
 ...
 *   debounceMs?: number;
 *   isPaused?: () => boolean;
 *   onPausedSchedule?: (trigger: { path: string; reason: string }) => void;
...
export function startHotUpdateWatch(options) {
  ...
  const schedule = (absPath) => {
    ...
    if (options.isPaused?.()) {
      options.onPausedSchedule?.({ path: relPath, reason: relPath });
      return;
    }
    lastTriggerPath = relPath;
    if (debounceTimer) clearTimeout(debounceTimer);
    debounceTimer = setTimeout(() => {
      debounceTimer = null;
      const path = lastTriggerPath ?? relPath;
