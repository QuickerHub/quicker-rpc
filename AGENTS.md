...
| 第三方 Agent 接入（MCP/skills） | 用户执行 `qkrpc agent setup`；见 [docs/agent-mcp-integration.md](docs/agent-mcp-integration.md) | — |
...
- **Shell 调 qkrpc**：QuickerAgent 的 `shell_exec` 会在 spawn 时**自动**把 `qkrpc` 目录 prepend 到 PATH（`agent-gui/lib/qkrpc-toolchain-env.mjs`）；`qkrpc mcp install` 也会写入工作区 `.vscode/settings.json` 的 `terminal.integrated.env`。仍优先 qkrpc MCP 工具。
- **qkrpc 连不上时**：先 **`qkrpc_wait`** / `qkrpc wait --json`（或 MCP `qkrpc_wait`）轮询等待；仍失败则告知用户检查 Quicker / 插件。**禁止** shell 连环探活，除非用户明确要求修复环境。
