...
| 改了什么 | Agent 必做 | 不要 |
|----------|-----------|------|
| `QuickerRpc/src/**`、`QuickerRpc/build.yaml`、`QuickerRpc/version.json` | 根目录 `pwsh -NoProfile -File ./build.ps1 -t`（或 `/hot-update`）；**必须等构建日志出现 plugin runaction + CLI Installed** | 只改代码不构建、让用户代跑、未重载就声称 Plugin 已修好 |
...
作用：停旧 `qkrpc serve` → 编译测试包并更新 `QuickerRpc_Run` version 变量 → **等待 1s 后** `quicker:runaction` **重载插件 DLL** → 发布 CLI → **安装到 `%LOCALAPPDATA%\Programs\qkrpc` 并写入用户 PATH** → 启新 serve（`9477`）。**未跑 `-t` 或未成功触发重载动作时，插件可能未加载。** 仅不想覆盖本机安装时加 `-SkipInstall`。
...
qkrpc subprogram publish --id <idOrName> --preflight --json
qkrpc subprogram publish --id <idOrName> --changelog "…" --json
...
**禁止**：未 `step-runner get` 就猜 `inputParams`（wire 写法：`paramKey` / `paramKey.file` / `paramKey.var`）；未 `subprogram get/search` 就猜 `callIdentifier`；未 `fa search` 就猜图标；patch 写与目录 Default 相同的普通参数。**禁止** `action publish`/`action update` 发布全局子程序（用 `subprogram publish`/`update`）。表达式优先 **quicker-eval-expression** skill / `sys:evalexpression`；兜底见 `guide get --topic implementation-fallback`。

**运行**：agent-gui `qkrpc_action` 要步骤输出用 `debug`（等价 `--trace`），不要用 `run`。CLI 退出码 0=成功、1=失败；脚本加 `--json`。

## 动作失败排错（trace + stack + log）

表达式/子程序/DLL 报错时 Agent **必须**走 trace，详见 [docs/agent-action-debug-workflow.md](docs/agent-action-debug-workflow.md)：

```powershell
qkrpc action run --id <guid> --trace --json   # MCP: qkrpc_action_debug
```

失败 JSON 优先读：`failureLocation.dataJsonPointer` → `stackTrace` → `logExcerpt`（自动摘 `quicker.log`）→ 完整 `%LocalAppData%\Quicker\logs\quicker.log`。

**禁止**：只看 `errorMessage` 摘要就改步骤；禁止用无 trace 的 `run` 排表达式错。
