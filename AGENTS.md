...
| 改了什么 | Agent 必做 | 不要 |
|----------|-----------|------|
| `QuickerRpc.Plugin/**`、`Console/**`、`Contracts/**`、`AgentModel/**` | 根目录 `pwsh -NoProfile -File ./build.ps1 -t`（或 `/hot-update`）；**必须等构建日志出现 plugin runaction** | 只改代码不构建、让用户代跑、未重载就声称 Plugin 已修好 |
