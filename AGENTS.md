...
**开发热更新（改 Plugin / qkrpc 后必做）**：Cursor 命令 **`/hot-update`**（或 `pwsh ./build.ps1 -t`）：停旧 serve → 编译 → 重载 Quicker 插件 → 启新 serve。**agent-gui / `start-agent-gui.ps1` 可保持运行**，不必关前端。细则见 `.cursor/commands/hot-update.md`、`.cursor/skills/quicker-rpc-build-test/SKILL.md`。
...
**Cursor**：热更新 **`/hot-update`**；改 **Plugin / Console / Contracts / AgentModel** → `quicker-rpc-build-test` skill。改 **agent-gui** → `quicker-agent-gui-frontend` skill（`dev_frontend_check`）。公开发布 **`/publish`** → `quicker-rpc-publish` skill。**动作页说明** → `quicker-action-doc` skill。
