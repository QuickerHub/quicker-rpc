...
| Meta | 怎么把学到的东西固化成子 skill | `action-authoring-src/skills/` + `learned-skills/registry.json` |
...
**默认产出子 skill 到 `action-authoring-src/skills/`，经 `docs:gen` 同步；registry 门控审核状态。**

1. 汇总 retro 中的重复卡点。
2. 对达标 pattern **创建或更新** `action-authoring-src/skills/<skillName>/`（见 §10）。
3. 将待审核条目标为 `registry.json` → `status: "review"`，写 **`learning-retro/YYYY-MM-DD-l4-summary.md`**。
4. 人类审核通过 → `registry.json` → `promoted`；确认 `npm run docs:gen`。
5. 可选：用户授权后改 **`prompt-tier0.src.md`**（父 skill 热路由 +1 行）。
...
2. 先写 `action-patterns/<slug>.md`，再写 **`docs/action-authoring-src/skills/<skillName>/`**（`SKILL.src.md` + `manifest.json`）。
3. 登记 `registry.json`（`status: draft`）→ `npm run docs:gen`。
4. **禁止**写到 `.cursor/skills/quicker-authoring-*`；**禁止**手改 `docs/skills/` 正文。
5. 审核通过：`registry` → `promoted`（内容已在 src，无需复制）。
...
| 子 skill（权威源） | `docs/action-authoring-src/skills/<skillName>/` | `docs:gen` → QuickerAgent on-demand |
| 子 skill 登记册 | `learned-skills/registry.json` | 审核 / 状态追踪 |
| 生成物 skill | `docs/skills/quicker-authoring-<slug>/` | setup + agent-gui catalog |
...
- L4: 至少 **1 轮** l4-summary + **≥1 个** 子 skill 写入 `action-authoring-src/skills/` 并登记 registry；`promoted` 需人类审核。
...
## 7. 写动作子 Skill（单源 → docs:gen）

```text
quicker-authoring（父，agent-gui 预加载）
├── quicker-eval-expression（兄弟）
├── quicker-authoring-selection-pipeline（子，action-authoring-src）
└── …
```

| 阶段 | 位置 | 说明 |
|------|------|------|
| **权威源** | `docs/action-authoring-src/skills/<skillName>/` | `SKILL.src.md` + `manifest.json` |
| **登记** | `learned-skills/registry.json` | draft / review / promoted |
| **深度参考** | `action-patterns/<slug>.md` | docs search |
| **生成物** | `docs/skills/<skillName>/` | `npm run docs:gen`；agent-gui on-demand |

规范全文：[`learned-skills/SPEC.md`](../../authoring-references/learned-skills/SPEC.md)。

### 7.1 创建时机（Phase 4 末尾）

pattern `done` 且 trace 通过后，按 SPEC §3 判定是否建子 skill；建则写 **src** + 更新 `registry.json` + `docs:gen`。

### 7.2 学习 Agent 自检

- [ ] `manifest.json` 的 `name` / `description` 正确
- [ ] 父 skill Scenario 表已加一行（新场景）
- [ ] **未**写到 `.cursor/skills/quicker-authoring-*`
- [ ] **未**手改 `docs/skills/`；已跑 `docs:gen`
- [ ] 未改 `PRELOADED_SKILL_NAMES`（除非用户授权）

### 7.3 人类审核

1. `registry.json` → `promoted` + `reviewedBy`
2. 确认 src 与 `docs/skills/` 一致（`npm run docs:gen`）
3. 批量校验：`node scripts/promote-learned-skills-from-registry.mjs --promote`
...
批末: 新 action-patterns md；新子 skill 写 action-authoring-src/skills + registry + docs:gen
