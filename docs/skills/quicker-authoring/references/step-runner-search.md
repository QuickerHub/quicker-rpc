...
用 `items[].key` 做 **`qkrpc_step_runner_get`**（**Agent 专用**，压缩 JSON、无 `icon`）。**action-editor UI** 用 `qkrpc step-runner get-ui`（含 `icon` 与完整 control 选项）。有 **`controlField`** 时，get **必须**传 **`controlField`** = **`controlField.value`**（多分支时从 **`controlFields`** 取对应项）；**禁止**猜其它 control 值。多数场景 **search 一次即可定 key + control**。
