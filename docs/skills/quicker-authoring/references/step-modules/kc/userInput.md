...
> **分类**：界面交互 · **来源**：KC 官方文档（`npm run docs:modules:gen`）· [userinput](https://getquicker.net/KC/Help/Doc/userinput)

**用途**：Prompt user for text input
...
- 用户输入模块仅能用于输入一个信息，如果需要同时输入多个内容，以及使用其它类型的输入方式（如下拉选择等），请使用《多字段表单》模块。
