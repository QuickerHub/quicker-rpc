...
- Smoke-test the action: qkrpc_action_debug({ id: "<guid>" }).

## Tr1 Discover schema (required before add/update)

```text
quicker_trigger({ action: "events", eventType?: "BrowserUrlChanged" })
```
...
Required for **add**: `eventType`, `actionIdOrName` (Guid or unique title). Resolve id: qkrpc_action_search({ query: "name", scope?: "agent" }).

```text
{{@ trigger.add eventType=BrowserUrlChanged action=<guid> params='{"UrlPattern":"https://github.com","OnlyActiveTab":true}' note="GitHub auto script"}}
```
...
```text
quicker_trigger({ action: "list", query?: "github", eventType?: "BrowserUrlChanged" })
quicker_trigger({ action: "update", id: "<guid>", params?: {"OnlyActiveTab":true}, enabled?: true, note?: "" })
```
