...
**`qkrpc_wait` / QuickerAgent `qkrpc_wait` tool** (agent-gui) auto-recovers:

1. Restart `qkrpc serve` if `:9477` is down (dev + bundled)
2. HTTP `wait` op with `quicker:runaction:aa5917ad-…?plugin` bootstrap (default)
3. CLI `qkrpc wait` fallback if serve still unreachable
