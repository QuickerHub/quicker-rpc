...
Use `items[].key` for **`qkrpc step-runner get`** (compressed JSON, no icon). **action-editor UI** uses `step-runner get-ui`. With **controlField**, get **must** pass **`--control-field <value>`** = **controlField.value**; NO guessing. Often **one search** fixes key + control.
