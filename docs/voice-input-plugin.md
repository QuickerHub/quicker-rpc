...
安装目录（**应用数据**，与 Agent 工作区分离；**完整路径、下载来源与校验见 [`docs/agent-gui-plugin-storage.md`](agent-gui-plugin-storage.md)**）：

```text
%LOCALAPPDATA%/QuickerAgent/plugins/voice-asr/    # 新安装
  manifest.json
  settings.json
  runtime/                 # quicker-voice-runtime.exe + _internal/
  models/sensevoice/         # model.int8.onnx, tokens.txt

# 旧版（仍兼容）: Documents/QuickerAgent/plugins/voice-asr/
```
...
2. **下载与校验** — 进度、可取消、sha256；**模型优先 ModelScope**，失败回退 Bitiful / GitHub 备用 zip；**Runtime 优先 Bitiful 镜像**，失败回退 GitHub Release（详见 [`docs/agent-gui-plugin-storage.md`](agent-gui-plugin-storage.md)）
...
| ④ | 插件 Host：安装 / 启停 / 状态 |
| ⑤ | 设置页 + 安装向导 |
| ⑥ | fork / 自研 Runtime → **`voice-asr-runtime/`**（Python，quicker-voice-v1） |

---

## 14. Runtime 子项目

实现位于 [`voice-asr-runtime/`](../voice-asr-runtime/)（可独立 `git init` fork）。
...
- 默认 **stub** 后端（无模型，验证协议）
- 可选 **sherpa-onnx**：`uv sync --extra sherpa` + 模型放入 `models/`
- agent-gui 联调：`pnpm voice:dev-server`
...
产品路径以 **`voice-asr-runtime/`** 自研 Runtime 为准（已从 CapsWriter 思路 fork，协议独立）。
...
- 默认工作区：`agent-gui/lib/default-working-directory.ts` → `Documents/QuickerAgent/workspace`
- 插件与依赖落盘：`docs/agent-gui-plugin-storage.md`（`%LOCALAPPDATA%/QuickerAgent/plugins/…`）
- Tauri 子进程模式：`agent-gui/src-tauri/src/lib.rs`（`BackendState`）
- CapsWriter 架构参考（非协议依赖）：WebSocket C/S、SenseVoice/Paraformer 模型
- Runtime 实现：[`voice-asr-runtime/`](../voice-asr-runtime/)
