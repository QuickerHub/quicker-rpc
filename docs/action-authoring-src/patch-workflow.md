...
仅元数据：顶层 `"icon"`（格式 **`action-icons`**）或 {{#ref patch.set-metadata.alt}}。
...
键名以 {{#ref step-runner.get.invoke}} 的 `schema.Inputs[].Key` 为准（有控制字段时传 {{#ref control-field.get}}，与 search 返回的 `controlFieldValue` 一致）；未知键不阻止保存，在响应 **`warnings[]`** 中列出合法键并可能给出相近建议（退出码仍为 0）。
...
冲突 → {{#ref action.get.retry}} 取新 `editVersion` 或 `force`。整页写入用 `"replace": true` 的 patch，或 `action replace`。仅当前 Quicker 配置内动作。
