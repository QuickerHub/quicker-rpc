...
{{#include-partial workflow-checklist-trigger}}
...
Required for **add**: `eventType`, `actionIdOrName` (Guid or unique title). Resolve id: {{@ action.search query=name}}.
...
{{#only-agent}}
```text
{{@ trigger.list query=github}}
{{@ trigger.update id=<guid> enabled=false}}
```
{{/only-agent}}
