...
{{#include-partial workflow-checklist-action-organization}}
...
{{@ profile.delete profileIds=[guid1,guid2]}}
```
{{/only-agent}}
{{#only-cli}}
```powershell
{{@ profile.delete id=<profileIdOrName>}}
{{@ profile.delete ids=<guid1>,<guid2>}}
```
{{/only-cli}}
