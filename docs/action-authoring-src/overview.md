...
## 编辑链路（必读顺序）
```text
阶段  目的                    {{#ref overview.pipeline.header}}
────  ──────────────────────  ─────────────────────────────────────────
...
## 常见校验（{{#ref errors.source}}）
| 场景 | 失败时参考 |
|------|------------|
| `inputParams` 键名 | {{#ref step-runner.get.invoke}}；未知键见 patch 的 `warnings[]` |
...
| 主题 | 何时读 |
|------|--------|
| **`authoring-workflow`** | {{#ref overview.workflow.when}} |
