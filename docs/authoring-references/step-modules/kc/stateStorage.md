...
> **分类**：文件与目录 · **来源**：KC 官方文档（`npm run docs:modules:gen`）· [statestorage](https://getquicker.net/KC/Help/Doc/statestorage)

**用途**：Read/write action state and badge
...
【版本1.3.0+】 如果不需要控制读取和写入状态的时间，也可以直接将变量设置为“作为状态使用”。在动作执行之前从状态加载变量值，在动作执行后将变量值保存在状态中。请参考：[https://getquicker.net/kc/help/doc/variables#4e0f2f58](https://getquicker.net/kc/help/doc/variables#4e0f2f58)
...
一个动作的所有状态使用json格式保存在 C:\Users\用户名\AppData\Local\Quicker\states 目录下，文件名为 "state_*动作ID*.json"。
...
专业版1.38.28 版本支持自动备份状态数据功能（仅备份，不是同步），需要自行开启相关选项。[参考文档](https://getquicker.net/kc/help/doc/statestorage#HO3Ao)
...
- 搜索Everything 使用状态存储everything软件的路径。在第一次运行时弹窗提示用户输入路径，下次就可以直接读取使用了。
- 示例：切换执行操作 第一次打开软件A，第二次打开软件B
- 示例：循环切换  循环切换列表中的内容
- 示例：状态存储 文件名每次加1
...
- 示例：计数器  每点击一次动作，计数加1
- 截图到桌面   用变量保存截图的保存位置。右键菜单可以打开设置位置的界面。
...
右键菜单数据的格式请参考：[https://www.yuque.com/quicker/help/action-custom-context-menu](https://www.yuque.com/quicker/help/action-custom-context-menu)
...
- https://getquicker.net/sharedaction?code=4aaaaf6c-1883-4c3a-9ed6-08d85d9ed340
