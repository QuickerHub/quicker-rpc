...
use crate::clipboard_history_plugin::{
    self, ClipboardHistoryPluginState, CLIPBOARD_HISTORY_ENABLED,
};
...
pub fn plugin_update(
    ...
    match plugin_id.as_str() {
        ...
        "clipboard-history" => {
            Err("clipboard-history runtime updates are not available yet".into())
        }
        _ => {
...
pub fn plugin_activate(
    ...
    if !activation::supports_on_demand(&events, &event) {
        return Err(format!(
            "plugin {plugin_id} does not support activation event {event}"
        ));
    }
