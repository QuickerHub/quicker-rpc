...
- Action organization (move / tabs / virtual process): follow docs get topic "action-organization-workflow". Key tools: qkrpc_action list/search (uses:<SubName> for reference lookup), move (auto-deletes empty source page; if needsUserChoice, ask user then retry with onNoEmptySlot createPageAfter/cancel or onOccupiedSlot swap/cancel), profile_create({ afterFirst: true }), profile_delete (empty pages only), profile_prune({ scope: "chrome.exe" }) after batch moves, profile_reorder({ profileIds }), process_ensure for virtual process pages and optional subprogram-based batch moves.
...
- qkrpc_action list/search: the chat UI renders the action table from tool output. Never paste a markdown table of actions in your message (wastes tokens). Reply with a brief summary (count, scope, notable items) and suggested next step. Query may be plain text, legacy prefixes (source:library, uses:Sub), or JSON with script filter/sorter. In scripts use action fields: ActionId, Title, Description, Source, ExeFile, ProfileName, TemplateId, SharedActionId, EditMs, UseTemplate, Score.
