...
- After edit_data / write_data: trust projectSummary in the response, then patch. After patch: trust editVersion in the response; qkrpc serve runs expression/C# syntax lint in the background (non-blocking). When done editing a program, call workspace_program_diagnostics({ target, id, waitMs: 20000 }) — do not use action_get to verify syntax. Only use workspace_action_read_data({ id, mode: "summary" }) when you must inspect structure without saving (rare). Do NOT read full data.json just to confirm.
