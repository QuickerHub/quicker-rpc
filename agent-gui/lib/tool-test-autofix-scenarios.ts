...
export const AUTO_FIX_SCENARIOS: AutoFixScenario[] = [
  {
    ...
    description:
      "在动作项目 data.json 中写入 {var} 但缺少 $$/$=，触发 valuePrefixWarnings；要求 Agent 自动修复并完成 patch。",
    userPrompt: [
      "请你在当前工作目录下，创建一个专用的测试动作项目并完成一次“造错→自动修复→再检查”的闭环。",
      "",
      "目标：模拟 agent-gui 在编辑动作时遇到 valuePrefixWarnings，然后自动修复并继续 patch。",
      "",
      "步骤要求：",
      ...
      "- 尝试写回/patch，预期会失败并返回 valuePrefixWarnings。",
      "- 读取 warnings 提示的切片位置，按建议自动修复为 \"$$Count: {n}\"（或改成 varKey 绑定也可）。",
      "- 重新写回/patch 直到 success=true。",
      "- 最后调用 workspace_program_diagnostics(waitMs=20000) 确认没有语法错误。",
      "",
  ...
  },
  {
    ...
    userPrompt: [
      ...
      "- patch 保存后，调用 workspace_program_diagnostics(waitMs=20000) 读取 issues，使用 location.read/locationSummary 定位问题。",
      "- 自动修复该表达式为合法表达式（例如 \"1 + 1\"），再 patch。",
      "- 再次调用 workspace_program_diagnostics(waitMs=20000)，直到 errorCount 为 0。",
    ].join("\n"),
  },
];
