...
| 你改了什么 | 做法 | 大约耗时 |
|-----------|------|----------|
| NSIS、`installer.nsh`、Next、业务逻辑 | `full` 或 `-ForceRetag` 全量重编 | ~10 min |
...
# ~3–8 min：+ next build + electron-prepare + staged verify（不编 NSIS）
...
| 参数 | 用途 |
|------|------|
| `-PreflightOnly` | 全量 Electron NSIS build 预检（阻塞） |
